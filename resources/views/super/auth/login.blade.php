...
<head>
    ...
    <title>スーパー管理者ログイン</title>
    @vite(['resources/css/app.css', 'resources/js/app.js'])
</head>
<body class="min-h-screen bg-gray-100 flex items-center justify-center">
    <div class="w-full max-w-sm bg-white rounded-xl shadow p-8">
        <h1 class="text-xl font-bold text-center mb-6">スーパー管理者ログイン</h1>
        ...
        <form method="POST" action="{{ route('super.login') }}">
            ...
            <button type="submit" class="w-full bg-indigo-600 text-white rounded-lg py-2 text-sm font-semibold hover:bg-indigo-700 transition">
                ログイン
            </button>
        </form>
    </div>
